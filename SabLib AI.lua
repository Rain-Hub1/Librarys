...
function Lib:Window(Info)
	...
	local OpenButton = Info.OpenButton or Enum.KeyCode.RightShift
	...
	local function makeDraggable(guiObject: GuiObject)
		...
		guiObject.InputEnded:Connect(function(input: InputObject)
			if input.UserInputType == Enum.UserInputType.MouseButton1 then
				isDragging = false
	...
	local function closeWindow()
		...
		local tween = TweenService:Create(MainFrame, tweenInfo, {
			BackgroundTransparency = 1,
			Position = UDim2.new(0.5, -200, 0.4, -250)
		})
	...
	if OpenButton ~= false then
		UserInputService.InputBegan:Connect(function(input: InputObject, gameProcessed: boolean)
			if not gameProcessed and input.KeyCode == OpenButton then
				if isOpen then
					closeWindow()
				else
					openWindow()
